...
dens_hist = np.zeros((len(cbins)-1,N_hist))
pol_hist = np.zeros((len(sbins)-1,N_hist))
nem_hist = np.zeros((len(sbins)-1,N_hist))
poln_hist = np.zeros((len(ebins)-1,N_hist))
nemn_hist = np.zeros((len(ebins)-1,N_hist))
...
for i in range(N_measure):
    ...
    for k in range(N_hist):
        ...
        dist = tree1.sparse_distance_matrix(tree2, max_distance=r_list[k],output_type='coo_matrix')
